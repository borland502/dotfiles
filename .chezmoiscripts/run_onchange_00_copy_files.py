...
if importlib.util.find_spec("xdg_base_dirs") and importlib.util.find_spec("plumbum"):
    ...
    sync_bin_cmd = rsync[
        "-avzPh",
        f"{xdg_data_home()}/automation/dasbootstrap/bin/",
        f"{Path.home()}/.local/bin/",
    ]
    logger.info(sync_bin_cmd())
    sync_lib_cmd = rsync[
        "-avzPh",
        f"{xdg_data_home()}/automation/dasbootstrap/lib/",
        f"{Path.home()}/.local/lib/",
    ]
    logger.info(sync_lib_cmd())
else:
    logging.warning(
        "onchange script failed due to missing imports.  If this is an initial setup then this is expected behavior"
    )
